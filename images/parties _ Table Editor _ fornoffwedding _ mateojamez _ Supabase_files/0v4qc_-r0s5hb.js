;!function(){try { var e="undefined"!=typeof globalThis?globalThis:"undefined"!=typeof global?global:"undefined"!=typeof window?window:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&((e._debugIds|| (e._debugIds={}))[n]="fc94c189-dd44-03c6-3310-41e24da6dd3b")}catch(e){}}();
...
where
  c.relkind = 'f'
`,I=t.z.object({id:t.z.number(),schema:t.z.string(),name:t.z.string(),comment:t.z.string().nullable(),foreign_server_name:t.z.string(),foreign_data_wrapper_name:t.z.string(),foreign_data_wrapper_handler:t.z.string(),columns:u.optional()}),$=t.z.array(I),q=t.z.optional(I),R=({includeColumns:e})=>n.safeSql`
...
from foreign_tables`;e.s(["default",0,{list:function({includeSystemSchemas:e=!1,includedSchemas:t,excludedSchemas:i,limit:l,offset:s,includeColumns:o=!0}={}){let c=R({includeColumns:o}),d=r(t,i,e?void 0:a.DEFAULT_SYSTEM_SCHEMAS);return d&&(c=n.safeSql`${c} where schema ${d}`),l&&(c=n.safeSql`${c} limit ${(0,n.literal)(l)}`),s&&(c=n.safeSql`${c} offset ${(0,n.literal)(s)}`),{sql:c,zod:$}},retrieve:function(e){return{sql:n.safeSql`${R({includeColumns:!0})} where ${function(e){if("id"in e&&e.id)return n.safeSql`${(0,n.ident)("id")} = ${(0,n.literal)(e.id)}`;if("name"in e&&e.name&&e.schema)return n.safeSql`${(0,n.ident)("name")} = ${(0,n.literal)(e.name)} and ${(0,n.ident)("schema")} = ${(0,n.literal)(e.schema)}`;throw Error("Must provide either id or name and schema")}(e)};`,zod:q}},zod:I}],254873);let L=n.safeSql`
...
`,e$=t.z.object({id:t.z.number(),name:t.z.string(),schema:t.z.string(),format:t.z.string(),enums:t.z.array(t.z.string()),attributes:t.z.array(t.z.object({name:t.z.string(),type_id:t.z.number()})),comment:t.z.string().nullable()}),eq=t.z.array(e$);e.s(["default",0,{list:function({includeArrayTypes:e=!1,includeSystemSchemas:t=!1,includedSchemas:i,excludedSchemas:l,limit:s,offset:o}={}){let c=eI;e||(c=n.safeSql`${c} and not exists (
      select from pg_type el
      where el.oid = t.typelem
        and el.typarray = t.oid
    )`);let d=r(i,l,t?void 0:a.DEFAULT_SYSTEM_SCHEMAS);return d&&(c=n.safeSql`${c} and n.nspname ${d}`),s&&(c=n.safeSql`${c} limit ${(0,n.literal)(s)}`),o&&(c=n.safeSql`${c} offset ${(0,n.literal)(o)}`),{sql:c,zod:eq}},zod:e$}],211309);let eR=n.safeSql`
select
  version(),
...
from approximation`}}],640696);let O=$.safeSql`
SELECT id, name, file_size_limit
FROM storage.buckets
WHERE file_size_limit IS NOT NULL
ORDER BY file_size_limit DESC
LIMIT ${(0,$.literal)(51)};
`;e.s(["LARGEST_SIZE_LIMIT_BUCKETS_COUNT",0,50,"getLargestSizeLimitBucketsSqlUnoptimized",0,O],517638),e.s(["getDatabaseExtensionsSQL",0,()=>$.safeSql`
SELECT
  e.name,
  n.nspname AS schema,
  e.default_version,
...
${l}`:l}],53336);let C=$.safeSql`pgmq_public`,F=$.safeSql`
...
`;e.s(["HIDE_QUEUES_FROM_POSTGREST_SQL",0,F,"QUEUES_SCHEMA",0,C,"getExposeQueuesSQL",0,({isNewerPgmqversion:e})=>{let t=e?$.safeSql`, conditional := '{}'::jsonb`:$.safeSql``,a=e?$.safeSql`, jsonb`:$.safeSql``;return $.safeSql`
  ...
  */
  ...
  on sequences
  to anon, authenticated, service_role;
`},"getQueuesExposePostgrestStatusSQL",0,()=>$.safeSql`
    SELECT exists (select schema_name FROM information_schema.schemata WHERE schema_name = '${C}');
  `],957386),e.s(["getTableRowsCountSql",0,({table:e,filters:t=[],enforceExactCount:a=!1,isUsingReadReplica:n=!1})=>{if(!e)return $.safeSql``;if(a){let a=new N.Query().from(e.name,e.schema??void 0).count();t.filter(e=>e.value&&""!==e.value).forEach(e=>{a=a.filter(e.column,e.operator,e.value)});let n=a.toSql(),i=n.endsWith(";")?n.slice(0,-1):n;return $.safeSql`select (${i}), false as is_estimate;`}{let a=new N.Query().from(e.name,e.schema??void 0).select();t.filter(e=>e.value&&""!=e.value).forEach(e=>{a=a.filter(e.column,e.operator,e.value)});let i=a.toSql(),r=i.endsWith(";")?i.slice(0,-1):i,l=new N.Query().from(e.name,e.schema??void 0).count();t.filter(e=>e.value&&""!=e.value).forEach(e=>{l=l.filter(e.column,e.operator,e.value)});let s=l.toSql(),o=s.endsWith(";")?s.slice(0,-1):s;return n?$.safeSql`
with approximation as (
    select reltuples as estimate
    from pg_class
    where oid = ${(0,$.literal)(e.id)}
)
select 
  case 
    when estimate > ${(0,$.literal)(5e4)} then (select -1)
    else (${o})
  end as count,
  estimate > ${(0,$.literal)(5e4)} as is_estimate
from approximation;
`:$.safeSql`
${q}
...
select 
  case 
    when estimate > ${(0,$.literal)(5e4)} then ${t.length>0?$.safeSql`pg_temp.count_estimate('${r.replaceAll("'","''")}')`:$.safeSql`estimate`}
    else (${o})
  end as count,
  estimate > ${(0,$.literal)(5e4)} as is_estimate
from approximation;
`}}],779262);let D=$.safeSql`
  ...
  $$
    ...
    END;
  $$;`;e.s(["getEntityDefinitionsSql",0,({schemas:e,limit:t=100})=>$.safeSql`
${D}
...
with records as (
  ...
  where
    ...
    and nc.nspname IN (${(0,$.joinSqlFragments)(e.map(e=>(0,$.literal)(e)),", ")})
...
from records r;
  `,"getTableDefinitionSql",0,({id:e})=>$.safeSql`
    ${D}
    ...
    from table_info t;
  `],538892);var z=e.i(248593),w=e.i(643135),x=e.i(9172);e.s(["getTablesPaginatedSql",0,({schema:e,includeColumns:t=!1,limit:a,afterOid:n})=>{let i=(0,w.filterByList)(e?[e]:void 0,void 0,e?void 0:z.DEFAULT_SYSTEM_SCHEMAS),r=i?$.safeSql`and nc.nspname ${i}`:$.safeSql``,l=t?$.safeSql`, columns as (${(0,x.getColumnsSql)({filter:{column:"oid",predicate:$.safeSql`in (select oid from page)`}})})`:$.safeSql``,s=t?$.safeSql`, ${(0,w.coalesceRowsToArray)("columns",$.safeSql`columns.table_id = tables.id`)}`:$.safeSql``;return $.safeSql`
    with page as (
      select
        c.oid,
        c.relname,
...
`},"getRemoveForeignKeySQL",0,({table:e,foreignKeys:t})=>{let a=t.map(t=>$.safeSql`ALTER TABLE IF EXISTS ${(0,$.ident)(e.schema)}.${(0,$.ident)(e.name)} DROP CONSTRAINT IF EXISTS ${(0,$.ident)(t.name)}`);return $.safeSql`${(0,$.joinSqlFragments)(a,";\n")};`}],788035);let H=$.safeSql`ROLE_IMPERSONATION_NO_RESULTS`;e.s(["ROLE_IMPERSONATION_NO_RESULTS",0,H,"ROLE_IMPERSONATION_SQL_LINE_COUNT",0,11,"getImpersonationSQL",0,({role:e,unexpiredClaims:t,sql:a})=>{var n;let i="postgrest"===e.type?void 0!==t?function({role:e,unexpiredClaims:t}){return $.safeSql`
select set_config('role', ${(0,$.literal)(e)}, true),
set_config('request.jwt.claims', ${(0,$.literal)(JSON.stringify(t))}, true),
...
set_config('request.headers', '{"accept": "*/*"}', true);
  `}({role:e.role,unexpiredClaims:t}):$.safeSql``:(n=e.role,$.safeSql`
    set local role ${(0,$.literal)(n)};
  `);return $.safeSql`
    ${i}
